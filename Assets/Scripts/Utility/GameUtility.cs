...
public class GameUtility
{
    ...
    public static void LengthCheck(UIInput input)
    {
        if (System.Text.Encoding.Default.GetByteCount(input.value) < 6)
        {
            FindDeepChild(input.gameObject, "wrong").gameObject.SetActive(true);
            FindDeepChild(input.gameObject, "right").gameObject.SetActive(false);
        }
        else
        {
            FindDeepChild(input.gameObject, "wrong").gameObject.SetActive(false);
            FindDeepChild(input.gameObject, "right").gameObject.SetActive(true);
        }
    }

    public static char InputValidate(string text, int charIndex, char addedChar)
    {
        if ((addedChar >= '0' && addedChar <= '9') || (addedChar >= 'A' && addedChar <= 'Z') || (addedChar >= 'a' && addedChar <= 'z')) return addedChar;
        return (char)0;
    }
